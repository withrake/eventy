...
export default function PhotoUploadWidget({setEditMode}) {
    ...
    function handleUploadImage() {
        setLoading(true);
        const filename = cuid() + '.' + getFileExtension(files[0].name); // we use the file ending from the file itself via util.js
        const uploadTask = uploadToFirebaseStorage(image, filename); //uploadTask provides us a snapshot that we can use
    ...
    return (
        <Grid>
            ...
            <Grid.Column width={4}>
                ...
                {files.length > 0 && //to avoid an error if there are no files for imagePreview
