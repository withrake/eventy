...
export default function FollowingTab({ profile, activeTab }) {
  ...
  useFirestoreCollection({
    query:
      activeTab === 3
        ? () => getFollowersCollection(profile.id) // if true we get the followers
        : () => getFollowingCollection(profile.id), // if not we must be in 4, so get following
    data: (data) =>
      activeTab === 3
        ? dispatch(listenToFollowers(data)) //data goes to the active tab
