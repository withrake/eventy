...
export default function UnauthModal({ history, setModalOpen }) {
  ...
  function handleClose() {
      ...
      if (history && prevLocation) {
          ...
          history.push('/events');
      } //when we cancel the dialogue, we go back to the previous page, instead of an empty page
    setOpen(false);
  }

  function handleOpenLoginModal(modalType) {
      dispatch(openModal({modalType})); //gets passed to the modalreducer to open the correct modal
      setOpen(false);
      setModalOpen(false);
  }
